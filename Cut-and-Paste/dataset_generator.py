...
def get_list_of_images(root_dir, N=1):
    ...
    '''
    img_list = [
        f for f in glob.glob(root_dir + '/*/*' + OBJECT_STRING)
...
def get_mask_file(img_file):
    ...
    '''
    mask_file = img_file.replace(OBJECT_STRING, MASK_STRING)
    return mask_file
            ...
            for i in range(len(blending_list)):
                ...
                if blending_list[i] == 'none' or blending_list[i] == 'motion':
                    ...
                    img_mask, img_src, offset_adj \
                        = pb.create_mask(img_mask.astype(np.float64),
                            img_target, img_src, offset=offset)
                    background_array = pb.poisson_blend(img_mask, img_src, img_target,
...
def generate_synthetic_dataset():
    ...
    '''
    img_files = get_list_of_images(OBJECT_DIR, NUM) 
