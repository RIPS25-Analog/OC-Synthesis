...
sys.path.append("/home/dhoban/comfyui/ComfyUI")
...
def load_checkpoints_n_models(background_image):
    ...
    controlnetloader_688 = controlnetloader.load_controlnet(
        control_net_name="sdx/diffusion_pytorch_model_promax.safetensors"
    )

    setunioncontrolnettype = NODE_CLASS_MAPPINGS["SetUnionControlNetType"]()
    setunioncontrolnettype_980 = setunioncontrolnettype.set_controlnet_type(
                type="canny/lineart/anime_lineart/mlsd",
                control_net=get_value_at_index(controlnetloader_688, 0),
    )
    ...
    ipadapter_loaded = ipadapterloader.load_models(get_value_at_index(differentialdiffusion_1043, 0), "STANDARD (medium strength)", provider="CUDA")
    ...
    return (
        get_value_at_index(setunioncontrolnettype_980, 0),
...
def preprocess_images_masks(loaded_image, loaded_mask):
    # floatslider = NODE_CLASS_MAPPINGS["FloatSlider"]()
    # floatslider_9 = floatslider.run(
    #     number=51.709, min_value=0, max_value=100, step=0.001
    # )

    # floatslider_10 = floatslider.run(
    #     number=58.565, min_value=0, max_value=100, step=0.001
    # )

    # floatslider_11 = floatslider.run(
    #     number=0.731, min_value=0, max_value=1, step=0.001
    # )
    ...
    # imagescaleby_1600 = imagescaleby.upscale(
    #     upscale_method="nearest-exact",
    #     scale_by=1.0000000000000002,
    #     image=loaded_image.unsqueeze(0),
    # )

    # imagepadforoutpaint = NODE_CLASS_MAPPINGS["ImagePadForOutpaint"]()
    # imagepadforoutpaint_1601 = imagepadforoutpaint.expand_image(
    #     left=0,
    #     top=0,
    #     right=0,
    #     bottom=0,
    #     feathering=40,
    #     image=get_value_at_index(imagescaleby_1600, 0),
    # )

    # imagescaleby_1602 = imagescaleby.upscale(
    #     upscale_method="nearest-exact",
    #     scale_by=1.0000000000000002,
    #     image=loaded_mask.unsqueeze(0),
    # )

    # imagepadforoutpaint_1603 = imagepadforoutpaint.expand_image(
    #     left=0,
    #     top=0,
    #     right=0,
    #     bottom=0,
    #     feathering=0,
    #     image=get_value_at_index(imagescaleby_1602, 0),
    # )

    # invertmask = NODE_CLASS_MAPPINGS["InvertMask"]()
    # invertmask_1615 = invertmask.invert(
    #     mask=get_value_at_index(imagepadforoutpaint_1603, 1)
    # )

    # imagetomask = NODE_CLASS_MAPPINGS["ImageToMask"]()
    # imagetomask_1605 = imagetomask.image_to_mask(
    #     channel="red", image=get_value_at_index(imagepadforoutpaint_1603, 0)
    # )

    # invertmask_1616 = invertmask.invert(
    #     mask=get_value_at_index(imagetomask_1605, 0)
    # )

    # maskcomposite = NODE_CLASS_MAPPINGS["MaskComposite"]()
    # maskcomposite_1612 = maskcomposite.combine(
    #     x=0,
    #     y=0,
    #     operation="multiply",
    #     destination=get_value_at_index(invertmask_1615, 0),
    #     source=get_value_at_index(invertmask_1616, 0),
    # )

    # invertmask_1647 = invertmask.invert(
    #     mask=get_value_at_index(maskcomposite_1612, 0)
    # )
    ...
    # layerutility_imageblendadvance_v2_7 = (
    #     layerutility_imageblendadvance_v2.image_blend_advance_v2(
    #         invert_mask=False,
    #         blend_mode="normal",
    #         opacity=100,
    #         x_percent=get_value_at_index(floatslider_9, 0),
    #         y_percent=get_value_at_index(floatslider_10, 0),
    #         mirror="None",
    #         scale=get_value_at_index(floatslider_11, 0),
    #         aspect_ratio=1,
    #         rotate=0,
    #         transform_method="lanczos",
    #         anti_aliasing=0,
    #         background_image=get_value_at_index(df_image_scale_by_ratio_657, 0),
    #         layer_image=get_value_at_index(imagepadforoutpaint_1601, 0),
    #         layer_mask=get_value_at_index(invertmask_1647, 0),
    #     )
    # )
...
def generate_collage_wrapper(params, w, h, scale_augment, rotation_augment, blending_list, allow_full_occlusion):
	''' Wrapper used to pass params to workers '''
	try:
		return generate_collage(*params, w=w, h=h, scale_augment=scale_augment, rotation_augment=rotation_augment,
...
def generate_collage(objects, img_file, anno_files, label_map, w, h, scale_augment, rotation_augment, blending_list, allow_full_occlusion):
...
def generate_data(args):
    ...
    # loaded_image = "collage.jpg"
    # loaded_mask = "collage_mask.png"
    # background_image = "/home/dhoban/RIPS25-AnalogDevices-ObjectDetection/src/Diffusion_CnP/background.jpg"
    # pos_prompt = ""
    # neg_prompt = ""
    ...
    write_yaml_file(args.output_dir, SELECTED_CLASSES, label_map)

    if not os.path.exists(args.output_dir):
        os.makedirs(args.output_dir)

    anno_dir = os.path.join(args.output_dir, 'labels')
    ...
    if args.parallelize:
        ...
        partial_func = partial(generate_collage_wrapper, w=WIDTH, h=HEIGHT, scale_augment=args.scale_augment, rotation_augment=args.rotation_augment, 
        ...
        for params in params_list:
            images_generated, masks_generated = generate_collage_wrapper(params, w=WIDTH, h=HEIGHT, scale_augment=args.scale_augment, rotation_augment=args.rotation_augment,
                                blending_list=BLENDING_LIST, allow_full_occlusion=args.allow_full_occlusion)
        ...
        for bg_idx in range(args.num_backgrounds):
            ...
            for idx, foreground, foreground_mask in zip(range(args.num_foregrounds), images_slice, masks_slice):
                ...
                for seed_idx, seed in enumerate(seeds):
                    ...
                    generated_background = generate_background(realvisxl, controlnet, cliptextencode_pos, cliptextencode_neg, edges, latent_image, vae, seed=seed, steps=args.steps)
                    ...
                    cv2.imwrite(os.path.join(args.output_dir, 'images', f"bg_{bg_idx}_fg_{idx}_seed_{seed_idx}.png"), final_image)

                    t7 = time.time()
                    logging.info(f"Time taken from: {t7 - t1:.2f} seconds")
