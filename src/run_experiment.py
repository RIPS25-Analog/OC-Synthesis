...
class YOLOfinetuner:
    ...
    def train_model(self):
        project_name = self.data_path.split('/')[-1].split('.')[0]
        results = self.model.train(data=self.data_path, epochs=self.epochs, freeze=self.freeze, project='runs/' + project_name)
        return results
