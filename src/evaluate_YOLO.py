...
class YOLOevaluator:
    def __init__(self, run_path, data_path=None, data_split='val'):
    ...
    def _get_data_path_from_model_yaml(self):
        ...
        if not data_path:
            raise ValueError("Data path not found in the model's YAML configuration. Please ensure the model was trained with a valid dataset.")
        return data_path
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, required=False, help='Path to the dataset configuration file (YAML). (Optional, will be fetched from the model YAML if not provided).')
