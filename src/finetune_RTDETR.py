...
class RTDETRFinetuner:
    def __init__(self, **kwargs):
        model_name = kwargs.get('model', 'rtdetr-l.pt')
        ...
        if 'save_dir' in kwargs:
            ...
            self.save_dir = os.path.join(wandb_runs_dir) # project_name/run_name gets added automatically
        os.makedirs(self.save_dir, exist_ok=True)

        if 'val' in kwargs:
            print(f"Validation during training is set to {kwargs['val']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train an RT-DETR model with specified parameters.',
                                      formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--model', type=str, default='rtdetr-l.pt', help='Path to the RT-DETR model file.')
    ...
    finetuner = RTDETRFinetuner(**train_kwargs)
